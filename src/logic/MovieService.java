...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		String uri = "mongodb://root:password@localhost:27017/?authSource=admin";
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		//TODO : implement
		FindIterable<Document>  result = movies.find(and(gt("votes", minVotes), gt("rating", minRating))).sort(orderBy(descending("votes"), descending("rating"))).limit(limit);
		return result;
	}
	...
	public FindIterable<Document> getByGenre(String genreList, int limit) {
		...
		//TODO : implement
		FindIterable<Document>  result = movies.find(in("genre", genres));
		return result;
	}
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		//TODO : implement
		Document keywordQuery = new Document("text", Pattern.compile(".*" + keyword + ".*"));;

		List<String> titles = new ArrayList<>();
		for (Document doc : tweets.find(keywordQuery).projection(Projections.fields(Projections.include("movie"), Projections.excludeId()))) {
			if (doc.getString("movie") != null) {
				titles.add(doc.getString("movie"));
			}
		}
		FindIterable<Document> result = movies.find(in("title", titles));
		return result;
	}
	...
	public FindIterable<Document>  getNewestTweets(int limit) {
		//TODO : implement
		FindIterable<Document>  result = tweets.find().sort(orderBy(descending("_id"))).limit(limit);
		return result;
	}
	...
	public FindIterable<Document>  getGeotaggedTweets(int limit) {
		//TODO : implement
		FindIterable<Document>  result = tweets.find(exists("coordinates"));
		return result;
	}
